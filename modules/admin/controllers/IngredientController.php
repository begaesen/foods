...
class IngredientController extends Controller
{
    ...
    {
        $find = ArrayHelper::map(FoodIngredient::find()->where("ingredient_id = $id")->all(),'id','food_id');
        
        if($find)
        {
            
            $model = Food::find()->where("id IN(".implode(',',$find).")")->all();
            if($model)
            {
                ...
                FoodIngredient::deleteAll("food_id IN(".implode(',',$find).")");
            }
    ...
    public function  actionVisible($id,$from)
    {
        $model = $this->findModel($id);
        if($model->visible == 0)
        {
        ...
        return $this->redirect([$from, 'id' => $id]);
